...
void Thread::search() {
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
          ...
          && !mainThread->stopOnPonderhit)
      {
          ...
          bool shuffling = rootPos.count<PAWN>() >= 10 && rootPos.rule50_count() >= 20 && rootPos.rule50_count() <= 80;
          double complexPosition = std::min(1.0 + (complexity - 261 - 100 * shuffling) / 1738.7, 1.5);
